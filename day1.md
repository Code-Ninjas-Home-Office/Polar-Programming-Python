...
- :tree: To start, type ``||scene:scene||`` and a dot operator ``||scene:.||``
- :tree: Type **set** then select ``||scene:set_background_image||`` from the **Code Completion Tool**.
- :image: Click on the palette then select **Gallery** to choose a background image. ![Scene](https://github.com/Code-Ninjas-Home-Office/arctic-code-quest/blob/master/images/DrawTool.png?raw=true "Draw Tool")
...
- :paper plane: To start, type a variable name such as ``||sprites:my_sprite||`` to declare your sprite. Then type an assignment operator ``||variables:=||``.
- :paper plane: Start typing the function ``||sprites:sprites.create||`` then select the function name from the **Code Completion Tool**.
- :image: Click on the palette to open the Gallery then select a sprite image. ![Scene](https://github.com/Code-Ninjas-Home-Office/arctic-code-quest/blob/master/images/DrawTool.png?raw=true "Draw Tool")
- :mouse pointer: Click DONE at the bottom right of the screen.
...
- :paper plane: Type a sprite's name and a dot operator ``||sprites:.||``. Select ``||sprites:set_position||`` from the **Code Completion Tool**.
...
- :game controller: To start, type ``||controller:controller||`` and a dot operator ``||controller:.||``, then select ``||controller:move_sprite||``.
...
- :paper plane: Type the name of your sprite, a dot operator ``||sprites:.||``, and select ``||sprites:set_stay_in_screen||``. 
...
- :paper plane: Type the Enemy sprite's variable name and a dot operator ``||sprites:.||``, then select ``||sprites:follow||`` from the **Code Completion Tool**.
...
- :circle: Inside, delete "pass" and type ``||game:game||`` and a dot operator ``||game:.||`` then select ``||game:game_over||`` from the **Code Completion Tool**.
